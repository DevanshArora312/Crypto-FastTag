...
import { SerializedPCD } from '../../node_modules/@pcd/pcd-types/dist/types/src'
...
export async function readFromLocalStorage(): Promise<AnonAadhaarState> {
  const json : string | undefined | null = await AsyncStorage.getItem('anonAadhaar')
  try {
...
function writeToLocalStorage(state: AnonAadhaarState) {
  ...
  AsyncStorage.setItem('anonAadhaar',serialize(state))
}
